...
interface LandingPageProps {
  onStart: () => void;
  hasApplied: boolean; // Page.tsx se status receive kiya
}
...
const LandingPage: React.FC<LandingPageProps> = ({ onStart, hasApplied }) => {
  ...
  // Progress logic: Applied = 100%, Logged In = 60%, Default = 0%
  const progressWidth = hasApplied ? '100%' : isSignedIn ? '60%' : '0%';

  return (
    <div className='relative pt-16 sm:pt-24 pb-12 sm:pb-16 px-4 overflow-x-hidden'>
      {/* Floating 3D Items */}
      <div className='absolute top-32 left-10 lg:left-20 z-0 hidden lg:block opacity-80 animate-float'>
        <span className='material-symbols-outlined text-6xl text-cyan-400 drop-shadow-[0_0_10px_rgba(34,211,238,0.5)]'>
          diamond
        </span>
      </div>
      <div className='absolute top-48 right-10 lg:right-20 z-0 hidden lg:block opacity-80 animate-float [animation-delay:1s]'>
        <span className='material-symbols-outlined text-6xl text-yellow-400 drop-shadow-[0_0_10px_rgba(250,204,21,0.5)]'>
          menu_book
        </span>
      </div>
        ...
        <div className='w-full max-w-[95%] sm:max-w-lg bg-[rgba(30,30,30,0.7)] backdrop-blur-sm border-2 border-white/10 p-5 sm:p-8 rounded-sm relative shadow-mc-card'>
          ...
          <div className='flex flex-col sm:flex-row items-center sm:justify-between mb-6 sm:mb-8 border-b-2 border-white/5 pb-4 gap-4 sm:gap-0'>
            <div className='flex items-center gap-3 w-full sm:w-auto'>
              <div className='h-12 w-12 sm:h-14 sm:w-14 bg-black/40 border-2 border-primary rounded-sm overflow-hidden flex items-center justify-center shrink-0'>
                {isSignedIn ? (
                  <img
                    alt='Profile'
            ...
            <div
              className={`bg-black/50 px-3 py-1 rounded border ${hasApplied ? 'border-primary' : 'border-white/10'} w-full sm:w-auto text-center`}>
              <span className='font-pixel text-base sm:text-lg text-primary animate-pulse'>
                {hasApplied ? 'APPLIED' : isSignedIn ? 'ONLINE' : 'OFFLINE'}
              </span>
            </div>
          </div>
          ...
          <div className='mb-8 sm:mb-10 relative mt-4 sm:mt-6'>
            <div className='w-full h-3 sm:h-3.5 bg-[#3a3a3a] border-2 border-black border-b-[#555] relative rounded-sm shadow-inner'>
              <div
                className={`h-full bg-primary shadow-[inset_0px_2px_0px_rgba(255,255,255,0.3),${hasApplied ? '0_0_20px_#39D339' : '0_0_10px_#39D339'}] rounded-sm relative transition-all duration-1000 ease-out`}
            ...
            <div className='flex justify-between mt-2 px-1'>
              <span className='font-pixel text-[10px] sm:text-sm text-gray-400 uppercase tracking-tight'>
                {hasApplied ? 'Protocol Complete' : 'Quest Authorization'}
              </span>
              <span className='font-pixel text-xs sm:text-sm text-primary'>
                {hasApplied ? '100%' : isSignedIn ? '60%' : '0%'}
              </span>
            </div>
          </div>
          ...
          {hasApplied ? (
            <div className='bg-[#1a2f1a] border-2 border-primary p-6 rounded-sm text-center animate-in fade-in zoom-in duration-500'>
              <h3 className='font-pixel text-2xl text-white mb-2 uppercase'>
                Submission Successful!
              </h3>
              <p className='text-zinc-300 text-sm font-display leading-relaxed'>
                Your application is successfully filed. Our High Council will
                review your logs. If selected, we will contact you via{' '}
                <span className='text-primary font-bold'>Discord</span>. Thank
                you for your interest in SpiralsMC!
              </p>
            </div>
          ) : (
            <button
      ...
      <div className='relative z-10 mt-16 sm:mt-24 bg-[#111] border-y border-white/10 py-10 sm:py-16 shadow-[0_0_50px_rgba(0,0,0,0.5)]'>
